...
import { Button, Dialog, DialogContent, DialogHeader, DialogTitle, ImageWithFallback } from '@cherry-studio/ui';
...
const categoryGradients: Record<ResourceCategory, string> = {
  agents: 'from-accent-violet/[0.06] via-blue-500/[0.03] to-transparent',
  assistants: 'from-foreground/[0.04] via-foreground/[0.02] to-transparent',
  knowledge: 'from-accent-blue/[0.06] via-accent-indigo/[0.03] to-transparent',
  mcp: 'from-accent-orange/[0.06] via-accent-amber/[0.03] to-transparent',
  skills: 'from-accent-amber/[0.06] via-accent-amber/[0.03] to-transparent',
  plugins: 'from-accent-pink/[0.06] via-accent-pink/[0.03] to-transparent',
};
...
export function PreviewModal({
  ...
  return (
    <Dialog open onOpenChange={(open) => { if (!open) onClose(); }}>
      <DialogContent className="bg-popover border border-border/30 shadow-2xl w-[540px] max-w-[540px] max-h-[85vh] flex flex-col overflow-hidden p-0 gap-0 [&>button:last-child]:hidden">
        ...
        {isKB(resource, category) ? (
          <div className="h-40 relative overflow-hidden flex-shrink-0">
            ...
            <Button variant="ghost" size="icon" onClick={onClose}
              className="absolute top-3 right-3 w-7 h-7 rounded-lg bg-foreground/30 backdrop-blur-sm text-white/70 hover:text-white hover:bg-foreground/40">
              <X size={13} />
            </Button>
            {/* Title overlay on cover */}
            <div className="absolute bottom-3 left-5 right-5">
              <div className="flex items-center gap-1.5 mb-1">
                <span className="text-xs px-1.5 py-px rounded-full bg-background/15 text-white/70 backdrop-blur-sm uppercase tracking-wider">{categoryLabels[category]}</span>
              </div>
            ...
            <div className="relative px-5 pt-5 pb-4">
              ...
              <div className="flex items-center justify-end gap-1 mb-4">
                <Button variant="ghost" size="icon"
                  className="w-6 h-6 rounded-lg text-muted-foreground/50 hover:text-foreground hover:bg-accent">
                  <Share2 size={11} />
                </Button>
                <Button variant="ghost" size="icon" onClick={onClose}
              ...
              <div className="flex items-start gap-3.5">
                <div className={`w-14 h-14 rounded-2xl flex items-center justify-center text-2xl flex-shrink-0 ring-1 ring-border/10 ${
                  isAgent(resource, category) ? 'bg-gradient-to-br from-accent-violet/10 to-blue-500/10' :
                ...
                <div className="flex-1 min-w-0 pt-0.5">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    <span className="text-xs px-1.5 py-px rounded-full bg-accent text-muted-foreground/50 uppercase tracking-wider flex-shrink-0">{categoryLabels[category]}</span>
                  </div>
                  <div className="flex items-center gap-2.5 mt-1">
                    ...
                    {(resource as any).version && (
                      <span className="text-xs text-muted-foreground/50 px-1.5 py-px rounded bg-accent/50">v{(resource as any).version}</span>
                    )}
          ...
          {(isAgent(resource, category) || isAssistant(resource, category)) && (
            <div>
              ...
              <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-accent/25 border border-border/10 w-fit">
                ...
                <span className="text-xs text-foreground">{models.find(m => m.id === resource.recommended_model)?.name}</span>
                <span className="text-xs text-muted-foreground/50 px-1.5 py-px rounded bg-accent/50">{models.find(m => m.id === resource.recommended_model)?.provider}</span>
              </div>
            </div>
          )}
            ...
            <div className="grid grid-cols-3 gap-2">
              {isAgent(resource, category) && (
                <div className="contents">
                  <StatCard icon={<Star size={12} className="text-warning/60" />} label="收藏" value={formatNumber(resource.stars)} color="amber" />
                  <StatCard icon={<Play size={12} className="text-accent-violet/60" />} label="运行次数" value={formatNumber(resource.runs)} color="violet" />
                  <StatCard icon={<Tag size={12} className="text-info/50" />} label="分类" value={resource.subcategory} color="blue" />
                </div>
              )}
              {isAssistant(resource, category) && (
                <div className="contents">
                  <StatCard icon={<Star size={12} className="text-warning/60" />} label="评分" value={String(resource.rating)} color="amber" />
                  <StatCard icon={<MessageCircle size={12} className="text-muted-foreground/60" />} label="对话数" value={formatNumber(resource.conversations)} color="neutral" />
                  <StatCard icon={<Tag size={12} className="text-info/50" />} label="分类" value={resource.subcategory} color="blue" />
                </div>
              )}
              {isKB(resource, category) && (
                <div className="contents">
                  <StatCard icon={<FileText size={12} className="text-info/60" />} label="文档" value={`${resource.docCount} 篇`} color="blue" />
                  <StatCard icon={<Tag size={12} className="text-accent-indigo/50" />} label="分类" value={resource.subcategory} color="indigo" />
                  <StatCard icon={<Calendar size={12} className="text-muted-foreground/40" />} label="更新时间" value={createdAt.slice(5)} color="gray" />
                </div>
              )}
              {(isMCP(resource, category) || isPlugin(resource, category)) && (
                <div className="contents">
                  <StatCard icon={<Download size={12} className="text-info/60" />} label="下载量" value={formatNumber(resource.downloads)} color="blue" />
                  <StatCard icon={<Tag size={12} className="text-accent-orange/50" />} label="分类" value={resource.subcategory} color="orange" />
                  <StatCard icon={<Calendar size={12} className="text-muted-foreground/40" />} label="更新时间" value={createdAt.slice(5)} color="gray" />
                </div>
              )}
              {isSkill(resource, category) && (
                <div className="contents">
                  <StatCard icon={<Zap size={12} className="text-warning/60" />} label="使用次数" value={formatNumber(resource.usageCount)} color="amber" />
                  <StatCard icon={<Tag size={12} className="text-warning/50" />} label="分类" value={resource.subcategory} color="amber" />
                  <StatCard icon={<Calendar size={12} className="text-muted-foreground/40" />} label="更新时间" value={createdAt.slice(5)} color="gray" />
                </div>
              )}
            </div>
          ...
          <Button size="inline"
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs ${
              isFavorited
                ...
                : 'bg-accent/50 text-muted-foreground/50 border border-border/15 hover:text-foreground hover:border-border/40'
            }`}
          >
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return (
    <div className="text-xs text-muted-foreground/50 uppercase tracking-wider mb-2">{children}</div>
  );
}
