...
import { X, Heart, ArrowUpDown, Trash2, Clock, ChevronRight } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { Button, SearchInput, Popover, PopoverTrigger, PopoverContent, EmptyState } from '@cherry-studio/ui';
...
const categoryColors: Record<ResourceCategory, string> = {
  agents: 'bg-accent-violet-muted text-accent-violet',
  assistants: 'bg-muted/50 text-foreground',
  knowledge: 'bg-info/10 text-info',
  mcp: 'bg-accent-orange-muted text-accent-orange',
  skills: 'bg-warning/10 text-warning',
  plugins: 'bg-accent-pink-muted text-accent-pink',
};
      ...
      {open && (
        <div className="contents">
          ...
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="absolute inset-0 z-[var(--z-overlay)] bg-foreground/10 backdrop-blur-[1px] rounded-xl"
            onClick={onClose}
          />
          ...
          <motion.div
            ...
            className="absolute top-1.5 right-1.5 bottom-1.5 w-[340px] z-[var(--z-modal)] bg-popover border border-border/20 rounded-2xl shadow-2xl flex flex-col overflow-hidden"
          >
            {/* Header */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-border/15 flex-shrink-0">
              <div className="flex items-center gap-2">
                <div className="w-6 h-6 rounded-lg bg-destructive/10 flex items-center justify-center">
                  <Heart size={11} className="text-destructive/70" fill="currentColor" />
                </div>
                <span className="text-sm text-foreground">我的收藏</span>
                <span className="text-xs text-muted-foreground/50 tabular-nums">{favorites.length}</span>
              </div>
              ...
              <div className="px-3 pt-3 pb-0 flex-shrink-0 space-y-2">
                ...
                <div className="flex items-center gap-1.5">
                  <SearchInput
                    ...
                    onChange={setSearch}
                    placeholder="搜索收藏..."
                    wrapperClassName="flex-1 px-2.5 py-1.5 rounded-lg border border-border/20 bg-accent/15"
                    clearable
                  />
                  {/* Sort dropdown */}
                  <Popover open={showSort} onOpenChange={setShowSort}>
                    <PopoverTrigger asChild>
                      <Button
                        variant="ghost"
                        size="icon-xs"
                        className={`flex items-center gap-1 px-2 py-1.5 rounded-lg text-xs border transition-all flex-shrink-0 ${
                          showSort
                    ...
                    <PopoverContent align="end" className="p-1 min-w-[110px]">
                  ...
                  {allCategoryFilter.map(cat => {
                    ...
                    return (
                      <Button
                        ...
                        className={`flex items-center gap-1 px-2 py-[3px] rounded-full text-xs whitespace-nowrap transition-all border ${
                          isActive
                      ...
                      >
                        <span>{cat.label}</span>
                        <span className={`tabular-nums ${isActive ? 'text-background/60' : 'text-muted-foreground/50'}`}>{count}</span>
                      </Button>
                    );
                  })}
                </div>
                  ...
                  <AnimatePresence>
                    {filtered.map((item, i) => (
                      <motion.div
                        ...
                        className="group flex items-center gap-2.5 px-3 py-2.5 rounded-xl hover:bg-accent/50 transition-colors cursor-pointer mb-0.5"
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className={`text-xs px-1.5 py-px rounded-full flex-shrink-0 ${categoryColors[item.category]}`}>
                              {item.categoryLabel}
                            </span>
                          </div>
                          <div className="flex items-center gap-1.5 mt-px">
                            <span className="text-xs text-muted-foreground/50">{item.author}</span>
                        ...
                        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                          <Tooltip content="取消收藏" side="left"><Button
                            ...
                            className="w-5 h-5 rounded-md text-muted-foreground/50 hover:text-destructive hover:bg-destructive/10"
                          >
                            <Trash2 size={10} />
                          </Button></Tooltip>
            ...
            {/* Footer count */}
            {favorites.length > 0 && (
              <div className="px-4 py-2.5 border-t border-border/15 flex-shrink-0">
