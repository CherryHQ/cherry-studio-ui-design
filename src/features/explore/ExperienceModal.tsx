      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute top-full left-0 mt-1.5 z-50 bg-popover border border-border/40 rounded-xl shadow-xl p-1 min-w-[220px] overflow-y-auto max-h-[280px] [&::-webkit-scrollbar]:w-[3px] [&::-webkit-scrollbar-thumb]:bg-border/30 [&::-webkit-scrollbar-thumb]:rounded-full"
          >
            {models.map(m => (
              <button
                key={m.id}
                onClick={() => { onSelect(m.id); setOpen(false); }}
                className={`w-full text-left px-2.5 py-[6px] rounded-lg text-[11px] transition-colors flex items-center gap-2 ${
...
export function ExperienceModal({ resource, type, onClose }: ExperienceModalProps) {
  ...
  return (
    <div ref={overlayRef} className="fixed inset-0 z-[500] flex items-center justify-center bg-black/50 backdrop-blur-sm"
      ...
      <motion.div
        ...
        className="bg-popover border border-border/30 rounded-2xl shadow-2xl w-[580px] max-h-[85vh] flex flex-col overflow-hidden"
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <span className="text-[13px] text-foreground truncate">{resource.name}</span>
                <span className="text-[9px] px-1.5 py-px rounded-full bg-accent text-muted-foreground/60 uppercase tracking-wide">
                  {type}
                </span>
              </div>
              <p className="text-[10px] text-muted-foreground/40 truncate mt-px">作者: {resource.author}</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-3 min-h-[300px] [&::-webkit-scrollbar]:w-[3px] [&::-webkit-scrollbar-thumb]:bg-border/30 [&::-webkit-scrollbar-thumb]:rounded-full">
          {messages.length === 0 && !thinking && (
            <div className="flex flex-col items-center justify-center h-full py-8">
              ...
              <p className="text-[12px] text-foreground/80 mb-1">{resource.name}</p>
              <p className="text-[11px] text-muted-foreground/40 text-center max-w-[300px] leading-relaxed">{resource.description}</p>
              {type === 'agent' && (
                <div className="flex items-center gap-1 mt-3 text-[10px] text-muted-foreground/30">
                  <Wrench size={9} />
          ...
          <AnimatePresence>
            {thinking && (
              ...
              >
                ...
                <span className="text-[11px] text-blue-600/70">{thinking}</span>
              </motion.div>
            )}
          </AnimatePresence>
          ...
          {toolLogs.length > 0 && (
            <div className="rounded-lg border border-border/15 bg-muted/[0.03] overflow-hidden">
              <div className="flex items-center gap-1.5 px-3 py-1.5 border-b border-border/10">
                ...
                <span className="text-[10px] text-muted-foreground/40">工具执行</span>
              </div>
        ...
        <div className="px-5 pb-3 pt-1 border-t border-border/15">
          <div className="flex items-end gap-2">
            <div className="flex-1 relative">
              <textarea
                ...
                rows={1}
                className="w-full px-3.5 py-2.5 rounded-xl border border-border/30 bg-accent/20 text-[12px] text-foreground placeholder:text-muted-foreground/30 outline-none focus:border-primary/25 focus:ring-1 focus:ring-primary/10 resize-none transition-all leading-relaxed"
            ...
            <button
              ...
              className="w-8 h-8 rounded-xl bg-primary text-primary-foreground flex items-center justify-center hover:bg-primary/90 transition-all disabled:opacity-30 flex-shrink-0 active:scale-95"
            >
              <Send size={12} />
            </button>
          </div>
          ...
          <div className="flex items-center justify-between mt-2.5">
            ...
            <button
              ...
              disabled={added}
              className={`flex items-center gap-1 text-[10px] px-2.5 py-1 rounded-lg transition-all ${
                added
