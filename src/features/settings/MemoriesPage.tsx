...
function EngineConfigPanel({ onClose }: { onClose: () => void }) {
  ...
  return (
    <motion.div
      ...
      className="absolute inset-y-0 right-0 w-[300px] bg-background border-l border-border/30 shadow-xl flex flex-col z-30 rounded-r-2xl"
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 [&::-webkit-scrollbar]:w-[2px] [&::-webkit-scrollbar-thumb]:bg-border/20">
        <div>
          ...
          <InlineSelect
            ...
            fullWidth
          />
          <p className="text-[8px] text-foreground/25 mt-1.5">用于从对话中分析并提取事实的模型。</p>
        </div>
        ...
        <div>
          ...
          <InlineSelect
            ...
            fullWidth
          />
          <p className="text-[8px] text-foreground/25 mt-1.5">用于将文本转换为向量进行存储和检索。</p>
        </div>
...
function SourceBadge({ source }: { source: 'chat' | 'manual' | 'auto' }) {
  const config = {
    chat: { label: 'Chat Conversation', icon: MessageCircle, color: 'text-foreground/30' },
    manual: { label: 'Manual', icon: PenLine, color: 'text-foreground/30' },
    auto: { label: 'Auto-Generated', icon: Sparkles, color: 'text-foreground/30' },
  };
  ...
  return (
    <div className="flex h-full min-h-0 relative">
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-0 relative">
        {/* Header */}
        <div className="flex items-center gap-3 px-5 py-3 flex-shrink-0 border-b border-foreground/[0.05]">
          {/* User info */}
        ...
        {/* Memory List */}
        <div className="flex-1 overflow-y-auto px-5 pb-4 space-y-2 [&::-webkit-scrollbar]:w-[3px] [&::-webkit-scrollbar-thumb]:bg-border/20">
          {filteredMemories.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-[9px] text-foreground/20">
                {search ? '尝试不同的搜索关键词' : '对话中的事实会被自动提取，或手动添加记忆。'}
              </p>
            </div>
          ) : (
            filteredMemories.map(memory => (
              <div
                key={memory.id}
                className="bg-foreground/[0.025] border border-foreground/[0.05] rounded-xl px-4 py-3 group hover:border-foreground/[0.1] transition-colors"
              >
                <div className="flex items-start justify-between mb-1.5">
                  <div className="flex items-center gap-2">
                    <span className="text-[9px]">🧠</span>
                    <span className="text-[9px] text-foreground/30">{memory.relativeTime}</span>
                  </div>
        ...
        <AnimatePresence>
          {showEngineConfig && (
            <div className="absolute inset-0 z-20" onClick={() => setShowEngineConfig(false)}>
        ...
        <AnimatePresence>
          {showNewUser && (
            <div className="absolute inset-0 z-20" onClick={() => setShowNewUser(false)}>
              <div className="fixed inset-0 bg-black/10 backdrop-blur-[1px] z-[-1]" />
        ...
        <AnimatePresence>
          {showAddMemory && (
            <div className="absolute inset-0 z-20" onClick={() => setShowAddMemory(false)}>
              <div className="fixed inset-0 bg-black/10 backdrop-blur-[1px] z-[-1]" />
              <AddMemoryPanel
