  ...
  return (
    <div className="max-w-lg space-y-6">
      <div>
        ...
        <p className="text-xs text-muted-foreground/55">选择知识库并配置检索参数</p>
      </div>

      {/* Linked */}
      <div>
        <label className="text-xs text-muted-foreground/60 mb-2 block">已关联知识库</label>
          ...
          <div className="space-y-1.5">
            {linkedItems.map(kb => (
              <div key={kb.id} className="flex items-center gap-3 px-3 py-2.5 rounded-xl border border-border/15 bg-accent/10 group hover:border-border/30 transition-colors">
                ...
                <Button variant="ghost" size="icon-xs" onClick={() => toggleLink(kb.id)} className="w-6 h-6 rounded-md text-muted-foreground/25 hover:text-destructive hover:bg-destructive/10 opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={10} /></Button>
              </div>
            ))}
          </div>
        )}
        <div className="relative mt-2">
          <Button variant="outline" size="xs" onClick={() => setShowKBPicker(!showKBPicker)}
            className="flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs text-muted-foreground/50 hover:text-foreground hover:bg-accent/30 transition-colors border border-border/15 hover:border-border/30">
            <Plus size={10} /> 添加知识库
          </Button>
          <AnimatePresence>
            {showKBPicker && (
                ...
                <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -4 }}
                  ...
                  <div className="relative mb-2">
                    ...
                    <Input value={kbSearch} onChange={e => setKbSearch(e.target.value)} placeholder="搜索知识库..."
                      className="w-full pl-6 pr-2 py-1.5 rounded-lg border border-border/15 bg-accent/10 text-xs outline-none focus:border-border/40 transition-all" />
                  </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="text-xs text-muted-foreground/60 mb-1.5 block">Top-K <span className="text-muted-foreground/35 ml-1">{topK}</span></label>
        ...
        <div>
          <label className="text-xs text-muted-foreground/60 mb-1.5 block">相似度阈值 <span className="text-muted-foreground/35 ml-1">{scoreThreshold.toFixed(2)}</span></label>
      ...
      <div className="flex items-start gap-2 px-3 py-2.5 rounded-xl bg-warning/5 border border-warning/15">
        ...
        <div>
          ...
          <p className="text-[9px] text-warning/40 mt-0.5">建议值：Top-K 3~8，阈值 0.6~0.8</p>
        </div>
      </div>
    </div>
  );
