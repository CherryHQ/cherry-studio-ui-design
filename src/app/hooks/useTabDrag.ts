...
// useTabDrag Hook
// ===========================
// Manages tab drag-and-drop interactions (dock to sidebar, detach to window).
...
export interface UseTabDragReturn {
  ...
  startTabDrag: (e: React.MouseEvent, tabId: string, callbacks: TabDragCallbacks) => void;
  startSidebarDrag: (e: React.MouseEvent, tabId: string, callbacks: SidebarDragCallbacks) => void;
}

interface TabDragCallbacks {
...
export function useTabDrag(): UseTabDragReturn {
  const [dragGhost, setDragGhost] = useState<DragGhostState | null>(null);
  const sidebarContainerRef = useRef<HTMLDivElement>(null);

  const startTabDrag = (e: React.MouseEvent, tabId: string, callbacks: TabDragCallbacks) => {
  ...
  const startSidebarDrag = (e: React.MouseEvent, tabId: string, callbacks: SidebarDragCallbacks) => {
    if (e.button !== 0) return;
    const startX = e.clientX;
    const startY = e.clientY;
    let dragging = false;
    ...
    const isOverSidebar = (cx: number, cy: number) => {
      ...
      const r = sidebarContainerRef.current.getBoundingClientRect();
      return cx >= r.left && cx <= r.right + 10 && cy >= r.top && cy <= r.bottom;
    };
