          ...
          {imageAttachments.map(att => (
            <div key={att.id} className="relative group rounded-lg overflow-hidden border border-border/30 w-[80px] h-[80px]">
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/60 to-transparent p-1">
                <span className="text-[8px] text-white/90 truncate block">{att.name}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {fileAttachments.length > 0 && (
        <div className="flex gap-1.5 flex-wrap mb-1.5 justify-end">
          {fileAttachments.map(att => (
            <div key={att.id} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-accent/40 border border-border/30">
              {getFileIcon(att.type)}
              <div className="min-w-0">
                <div className="text-[10px] text-foreground/80 truncate max-w-[120px]">{att.name}</div>
                <div className="text-[8px] text-muted-foreground/50">{att.size}</div>
              </div>
            </div>
          ))}
        </div>
