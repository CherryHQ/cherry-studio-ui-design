...
function DialogOverlay({ className, ...props }: React.ComponentProps<typeof DialogPrimitive.Overlay>) {
  return (
    <DialogPrimitive.Overlay
      ...
      className={cn(
        // Bumped from v2's default z-[80] to the project-wide modal
        // overlay token (z-overlay = 400) so the dim covers the app
        // sidebar, not just the main content area.
  ...
  return (
    <DialogPortal data-slot="dialog-portal">
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          // Same z-bump as DialogOverlay above — z-[var(--z-modal)]
          // (500) sits above the dimmed app sidebar and main content.
          'bg-card text-card-foreground data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[50%] left-[50%] z-[var(--z-modal)] grid w-full max-w-[calc(100%-2rem)] translate-x-[-50%] translate-y-[-50%] gap-4 rounded-3xl border-0 p-6 shadow-xl duration-200 sm:max-w-lg',
          className
        )}
        {...props}>
        {children}
