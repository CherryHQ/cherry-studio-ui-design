...
import {
  Button, Input, Dialog, DialogContent, Label,
...
// Cold-tone color pool for demo mock data (derived from theme chart palette)
const COLOR_POOL = [
  "#3582FF", "#6366F1", "#8755E9", "#06b6d4", "#64748b",
  "#818cf8", "#a78bfa", "#22d3ee", "#94a3b8", "#7c3aed",
  "#4f46e5", "#0ea5e9", "#6d28d9", "#475569", "#2563eb",
] as const
...
// ===========================
const MINI_APPS = [
  { id: "chatgpt", name: "ChatGPT", color: "#06b6d4", initial: "G", category: "chat" as const },
  { id: "gemini", name: "Gemini", color: "#4285f4", initial: "✦", category: "chat" as const },
  { id: "claude", name: "Claude", color: "#818cf8", initial: "C", category: "chat" as const },
  ...
  { id: "groq", name: "Groq", color: "#4f46e5", initial: "G", category: "tool" as const },
  { id: "siliconflow", name: "SiliconFlow", color: "#3b82f6", initial: "SF", category: "tool" as const },
  { id: "hailuoai", name: "海螺", color: "#7c3aed", initial: "螺", category: "creative" as const },
  ...
  { id: "n8n", name: "n8n", color: "#475569", initial: "n8n", category: "tool" as const },
  { id: "notebooklm", name: "NotebookLM", color: "#0ea5e9", initial: "NL", category: "tool" as const },
  { id: "grok", name: "Grok", color: "#1a1a2e", initial: "X", category: "chat" as const },
  { id: "wps", name: "WPS灵犀", color: "#6d28d9", initial: "W", category: "creative" as const },
  { id: "baichuan", name: "百小应", color: "#0ea5e9", initial: "百", category: "chat" as const },
  ...
  { id: "minimax", name: "MiniMax", color: "#22d3ee", initial: "M", category: "chat" as const },
  ...
  { id: "suno", name: "Suno", color: "#a78bfa", initial: "S", category: "creative" as const },
  ...
  { id: "tavily", name: "Tavily", color: "#64748b", initial: "T", category: "search" as const },
        ...
        <div className="px-6 pt-2 pb-0.5 flex items-center gap-0.5">
          {CATEGORY_TABS.map(tab => (
            <Button
              ...
              className={`h-auto px-2.5 py-1 rounded-[10px] text-xs tracking-tight font-normal ${activeCategory === tab.id ? "bg-accent text-foreground font-medium" : "text-muted-foreground/50 hover:text-foreground"}`}
            >
              {tab.label}
            </Button>
          ))}
          ...
          <div className="max-w-3xl mx-auto">
            {displayedApps.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 text-muted-foreground/30">
                ...
                <span className="text-sm tracking-tight">{searchTerm ? "没有找到匹配的小程序" : "暂无小程序"}</span>
              </div>
            ) : (
              <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 gap-x-2 gap-y-4">
                {displayedApps.map(app => (
                  <ContextMenu key={app.id}>
                    <ContextMenuTrigger asChild>
                      ...
                      >
                        <div className="transition-transform group-hover:scale-110 shadow-sm">
                          <div
                            className="w-11 h-11 rounded-[12px] flex items-center justify-center text-primary-foreground text-xs font-medium"
                            style={{ background: app.color }}
                          >
                            {app.initial}
                          </div>
                        </div>
                        <span className="text-xs text-muted-foreground group-hover:text-foreground transition-colors truncate max-w-[100px] tracking-tight">
                          {app.name}
                        </span>
                        {app.isCustom && (
                    ...
                    <ContextMenuContent>
                      <ContextMenuItem onClick={() => alert(`打开应用: ${app.id}`)}>
                        <Globe size={12} className="mr-2 text-muted-foreground" /> 打开
                      </ContextMenuItem>
                      <ContextMenuItem onClick={() => setHiddenApps(prev => new Set(prev).add(app.id))}>
                        <EyeOff size={12} className="mr-2 text-muted-foreground" /> 隐藏
                      </ContextMenuItem>
                      <ContextMenuItem onClick={() => navigator.clipboard.writeText(`https://app.cherry.ai/${app.id}`)}>
                        <Copy size={12} className="mr-2 text-muted-foreground" /> 复制链接
                      </ContextMenuItem>
                      {isCustomApp(app.id) && (
                        <>
                          <ContextMenuSeparator />
                          <ContextMenuItem onClick={() => { const a = customApps.find(c => c.id === app.id); if (a) openEditDialog(a) }}>
                            <PenLine size={12} className="mr-2 text-muted-foreground" /> 编辑
                          </ContextMenuItem>
                          <ContextMenuItem onClick={() => deleteApp(app.id)} className="text-destructive">
                ...
                <Button variant="ghost" onClick={() => setShowAddDialog(true)} className="flex flex-col items-center gap-1.5 h-auto p-1.5 font-normal tracking-normal group">
                  ...
                  <span className="text-xs text-muted-foreground/40 group-hover:text-muted-foreground transition-colors tracking-tight">自定义</span>
                </Button>
              </div>
            )}
          </div>
      ...
      <Sheet open={showSettings} onOpenChange={setShowSettings}>
        <SheetContent side="right" className="w-[320px] p-0">
          <SheetHeader className="px-4 py-3 border-b border-border/30">
            <SheetTitle className="text-md tracking-tight">管理小程序</SheetTitle>
          </SheetHeader>
          <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4">
            {/* Visible apps */}
            <div>
              <p className="text-xs text-muted-foreground/50 mb-2 tracking-tight">已显示 ({visibleApps.length})</p>
                ...
                {visibleApps.map(app => (
                  <div key={app.id} className="flex items-center justify-between px-2.5 py-1.5 rounded-[10px] hover:bg-accent/50 transition-colors">
                    <div className="flex items-center gap-2.5">
                      <div className="w-7 h-7 rounded-[8px] flex items-center justify-center text-primary-foreground text-xs font-medium" style={{ background: app.color }}>{app.initial}</div>
                      <span className="text-xs text-foreground tracking-tight">{app.name}</span>
                    </div>
            ...
            {hiddenAppList.length > 0 && (
              <div>
                <p className="text-xs text-muted-foreground/50 mb-2 tracking-tight">已隐藏 ({hiddenAppList.length})</p>
                <div className="space-y-0.5">
      ...
      <Dialog open={showAddDialog} onOpenChange={open => { if (!open) { setShowAddDialog(false); setEditingApp(null); setFormName(""); setFormUrl("") } }}>
        <DialogContent className="max-w-sm">
          <div className="space-y-4">
            <h3 className="text-base font-medium tracking-tight">{editingApp ? "编辑小程序" : "添加自定义小程序"}</h3>
