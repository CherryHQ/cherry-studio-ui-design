...
export function NewTabDialog({ open, search, onSearchChange, onSelect, onClose, hiddenApps, setHiddenApps, appOrder, setAppOrder, dialogAppIcons, dialogFilterTabs, newTabHistoryItems, newTabFileItems, dialogQuickActions }: {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) onClose() }}>
      <DialogContent data-slot="new-tab-dialog" className="max-w-[520px] p-0 top-[8%] translate-y-0 overflow-hidden flex flex-col tracking-tight" showCloseButton={false}>
        {/* Search input */}
            ...
            {visibleApps.map((app) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="text-[11px] text-muted-foreground/70 group-hover:text-foreground leading-none">{app.label}</span>
                </Button>
              );
            })}
          </div>
        ...
        <div className="flex items-center gap-1.5 px-4 py-2 border-b border-border/50 overflow-x-auto [&::-webkit-scrollbar]:hidden">
          ...
          <Button variant="ghost" size="icon-xs" className="text-muted-foreground/40 hover:text-muted-foreground"><SortAsc size={12} /></Button>
          <Button variant="ghost" size="icon-xs" className="text-muted-foreground/40 hover:text-muted-foreground"><CalendarDays size={12} /></Button>
        </div>

        {/* Content */}
        <div className="max-h-[380px] overflow-y-auto [&::-webkit-scrollbar]:hidden" role="list" aria-live="polite">
          {manageMode ? (
            /* Manage Mode */
                ...
                {orderedApps.map((app, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-[13px] text-foreground flex-1">{app.label}</span>
                      <Button
                        variant="ghost"
                        size="icon-sm"
